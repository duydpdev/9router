...
export async function enableTailscale(localPort = 20128) {
  log.info("Tailscale", `enable start (port=${localPort})`);
  ...
  try {
    ...
    log.debug("Tailscale", "daemon ready");
    ...
    log.debug("Tailscale", `loggedIn=${loggedIn}`);
    if (!loggedIn) {
      const loginResult = await startLogin(tsHostname);
      if (loginResult.authUrl) {
        log.info("Tailscale", `needs login, authUrl=${loginResult.authUrl}`);
        return { success: false, needsLogin: true, authUrl: loginResult.authUrl };
      }
      log.debug("Tailscale", "login resolved alreadyLoggedIn");
    }
    throwIfCancelled(token);
    ...
    let result;
    try {
      log.debug("Tailscale", "starting funnel");
      result = await startFunnel(localPort);
    } catch (e) {
      log.error("Tailscale", `funnel error: ${e.message}`);
      ...
      if (/NoState|unexpected state|not logged in|Logged ?out|NeedsLogin/i.test(e.message || "")) {
        log.warn("Tailscale", "retry via startLogin");
        const loginResult = await startLogin(tsHostname);
    ...
    if (result.funnelNotEnabled) {
      log.warn("Tailscale", `funnel not enabled, enableUrl=${result.enableUrl}`);
    ...
    if (!isTailscaleLoggedIn() || !isTailscaleRunningStrict()) {
      log.error("Tailscale", "strict probe failed (device removed?)");
      stopFunnel();
    ...
    log.info("Tailscale", `funnel up: ${result.tunnelUrl}`);
    ...
    try {
      ...
      log.warn("Tailscale", "health check timed out, will retry via watchdog");
    }
    log.info("Tailscale", `enable success (reachable=${reachableNow})`);
    return { success: true, tunnelUrl: result.tunnelUrl };
  } catch (e) {
    log.error("Tailscale", `enable error: ${e.message}`);
    throw e;
  } finally {
